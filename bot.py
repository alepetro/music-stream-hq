...
bot = telebot.TeleBot(TELEGRAM_TOKEN, parse_mode="HTML", threaded=True, num_threads=16)
...
class _PooledConnection(sqlite3.Connection):
    """Thread-local pooled connection: close() is a no-op so hot paths reuse it."""

    def close(self):  # noqa: D401 — intentional no-op, real close at interpreter exit
        pass
...
def get_db() -> sqlite3.Connection:
    """One persistent SQLite connection per thread (WAL allows concurrent readers).

    Avoids re-opening the DB + re-running PRAGMAs on every call, which was a
    measurable cost on hot paths (every message hits get_user_lang/upsert_user).
    """
    ...
    if conn is None:
        ...
        # Transaction hygiene: if a previous operation on this thread errored
        # mid-write and was swallowed, roll back so we never leak an open
        # transaction (which would hold write locks and bleed across requests).
        try:
            if conn.in_transaction:
                conn.rollback()
        except sqlite3.Error:
            # Connection is unusable — recreate it
            try:
                conn.real_close()
            except Exception:
                pass
...
# query (lowercased) → {"ts": epoch, "results": [...]}. Repeated searches are instant.
_search_result_cache: dict[str, dict] = {}
_search_cache_lock = threading.Lock()
SEARCH_CACHE_TTL = 300  # seconds
SEARCH_CACHE_MAX = 500  # hard cap on distinct cached queries
...
def search_youtube(query: str, max_results: int = SEARCH_POOL) -> list[dict]:
    ...
    opts = {
        ...
        "skip_download": True, "socket_timeout": 10, "noplaylist": True,
    }
    results = []
    ...
    if results:
        with _search_cache_lock:
            if len(_search_result_cache) >= SEARCH_CACHE_MAX:
                # Drop the oldest half — O(n log n) but rare and bounded
